import React from 'react'

const EditProfile = props => {
  const { hexFill } = props

  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg">
      <path
        d="M9 4C10.3512 5.27836 11.2146 6.00038 13.1617 6C14.1376 6.00025 15.1034 5.81902 16 5.46737"
        stroke={hexFill}
        strokeWidth="2"
        strokeLinejoin="round"
        strokeLinecap="round"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12.1025 12C14.864 12 17.1025 9.76142 17.1025 7C17.1025 4.23858 14.864 2 12.1025 2C9.34112 2 7.10254 4.23858 7.10254 7C7.10254 9.76142 9.34112 12 12.1025 12Z"
        stroke={hexFill}
        strokeWidth="2"
        strokeLinejoin="round"
        strokeLinecap="round"
      />
      <path
        d="M3 22C3 17.0294 7.02944 13 12 13C12.6874 13 13.3568 13.0771 14 13.223C14.7013 13.3822 15.3715 13.6232 16 13.9355"
        stroke={hexFill}
        strokeWidth="2"
        strokeLinejoin="round"
        strokeLinecap="round"
      />
      <path
        d="M22.5768 14.4231C22.0082 13.8571 21.0886 13.8594 20.5228 14.4282C20.5199 14.4312 20.5169 14.4341 20.514 14.4371L14.728 20.2233L14 23.0001L16.7768 22.2723L22.5628 16.4861C23.1362 15.9259 23.147 15.0069 22.5869 14.4334C22.5836 14.4299 22.5802 14.4265 22.5768 14.4231Z"
        stroke={hexFill}
        strokeWidth="2"
        strokeLinejoin="round"
        strokeLinecap="round"
      />
    </svg>
  )
}

export default EditProfile
