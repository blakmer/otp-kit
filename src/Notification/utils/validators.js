...
const isReactElement = value =>
  isFunctionComponent(value) || React.isValidElement(value)
...
export function validateTransition(notification, transition) {
  ...
  if (!isNull(duration) && !isNumber(duration)) {
    throw new Error(
      TRANSITION_DURATION_NUMBER.replace('transition', transition)
    )
  }
  if (!isNull(timingFunction) && !isString(timingFunction)) {
    throw new Error(
      TRANSITION_TIMING_FUNCTION.replace('transition', transition)
    )
  }
